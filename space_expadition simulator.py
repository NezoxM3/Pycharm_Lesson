...
class Astronaut:
    ...
    def survive_in_space(self):
        if self.oxygen <= 10:
            print("Запас кисню низький!")
        if self.energy <= 10:
            print("Рівень енергії критично низький!")
...
class Spaceship:
    ...
    def travel(self, planet):
        if self.fuel >= 10:
            ...
            print("Немає достатньо пального")
...
class Alien:
    ...
    def negotiate(self, astronaut):
        if randint(0, 1):
            ...
            print(f"{self.species} дружні")
        else:
            print(f"{self.species} не дружні")


ship = Spaceship("Galactic Explorer")
astronaut = Astronaut("Max", ship)
planet = Planet("BTW-348", ["залізо", "метановий кристал", "титан"], True, 1.5, [Alien("Batwerians", "дружній", 5, ["енергитична куля", "Вода"])])
